...
class Portal extends PositionComponent with HasGameReference<DarkbladeGame> {
  ...
  void render(Canvas canvas) {
    ...
    canvas.drawArc(
        ...
        pi, pi, false, arch);
    canvas.drawLine(Offset(rect.left, rect.top + 10),
        Offset(rect.left, rect.bottom), arch);
    canvas.drawLine(Offset(rect.right, rect.top + 10),
        Offset(rect.right, rect.bottom), arch);

    if (unlocked) {
      // Swirling energy.
      final glow = Paint()
        ..color = const Color(0x557B2FF2)
